...
char MAX30102getid() {
	return i2c_recv_byte(MAX30102_PART_ID);
}

void MAX30102starttemp(void) {
	i2c_send_byte(MAX30102_INTR_EN2,0x02); //enable temp interrupt
	i2c_send_byte(MAX30102_TEMP_CONFIG,0x01); //start temp
}

void MAX30102init() {
	unsigned char uc_i;
	if (i2c_recv_byte(MAX30102_PART_ID)!= MAX30102_ID) return;
	i2c_send_byte(MAX30102_MODE_CONFIG,0x40); //reset
	for (uc_i=0; uc_i<255; uc_i++) wait2ms();
	i2c_send_byte(MAX30102_FIFO_CONFIG,0x40); //sample avg=4, fifo rollover=false, fifo almost full=32
	i2c_send_byte(MAX30102_MODE_CONFIG,0x03); //0x02 for Red only, 0x03 for SpO2 mode 0x07 for multimode LED
	i2c_send_byte(MAX30102_SPO2_CONFIG,0x27); //SPO2_ADC=4096nA, SPO2 sample rate=100Hz, pulseWidth=411us
	i2c_send_byte(MAX30102_LED1_PA,0x1F); //LED1(IR) 6.2 mA
	i2c_send_byte(MAX30102_LED2_PA,0x1F); //LED2(red) 6.2 mA
	//MAX30102starttemp();
}

unsigned char MAX30102read(unsigned char* pcdata) {
	unsigned char ucDataReady = i2c_recv_byte(MAX30102_FIFO_WR_PTR) - i2c_recv_byte(MAX30102_FIFO_RD_PTR);
	if (ucDataReady) {
		i2c_recv_burst(MAX30102_FIFO_DATA, pcdata, 6);
		pcdata[0] &= 0x03;
		pcdata[3] &= 0x03;
	} 
	return (ucDataReady);
...
unsigned char MAX30102gettemp(char* ctemp) {
	unsigned char cRdy = i2c_recv_byte(MAX30102_INTR_STAT2) & 0x02;
	if (cRdy) *ctemp = i2c_recv_byte(MAX30102_TEMP_INT);
	return cRdy;
}
...
//	}
	...
	uc_prevsig = SIG_NONE;
	uc_x = 0; uc_y = 0;
	lcd_char_scale(&uc_x, &uc_y, 0x3B, 3);
	//lcd_string_scale(&uc_x, &uc_y, "H   ", 3);
	uc_x = 0; uc_y = 3;
	lcd_string_scale(&uc_x, &uc_y, "O", 3);
	lcd_pos(10,5);
	lcd_string(" ", 0);
	lcd_pos(11,5);
	lcd_string("2", 0);
	...
	while (1) {
		...
		asm("CLRWDT");
		//temperature
//		if (MAX30102gettemp(&uc_i)) {
//			lcd_pos(0,0);
//			lcd_printDec2(uc_i, 0);
//			MAX30102starttemp();
//		}
		//disp raw read
//		lcd_pos(0,4);
//		for (uc_i=0; uc_i<3; uc_i++) lcd_printHex(pcdata[uc_i], 2);//IR
//		lcd_pos(0,5);
//		for (uc_i=0; uc_i<3; uc_i++) lcd_printHex(pcdata[uc_i+3], 2);//Red
		slSig1 = (((short long)pcdata[0]) << 16) | (((short long)pcdata[1]) << 8) | pcdata[2];
		slSig2 = (((short long)pcdata[3]) << 16) | (((short long)pcdata[4]) << 8) | pcdata[5];
		//disp wave form
		sl_wave = (slSig1 - slSigBase) / slSigScale + DISP_MARGIN;
		if (sl_wave < 0) uc_wave = 0; else if (sl_wave >= DISP_HEIGHT) uc_wave = DISP_HEIGHT-1; else uc_wave = sl_wave;
		uc_j = ui_time % DISP_WIDTH;
		...
		if ((uc_sig == SIG_DESCENT)&&(uc_prevsig == SIG_ASCENT)) {
			...
			//blink heart icon
			uc_x = 0; uc_y = 0;
			lcd_char_scale(&uc_x, &uc_y, 0x3A, 3);
			//calc beta
			fIR = ulIR / ucSum;
			...
			if (uiStore >= DIM_STORE+5) {
				...
				uc_x = 16; uc_y = 0;
				lcd_printDec3scale(&uc_x, &uc_y, ucSum, 3);
				fRed = 0;
		...
		//update cycle
		ui_time++;
		if (ui_time % DISP_WIDTH == 0) {
			slSigBase = slWaveMin;
			slSigScale = (slWaveMax - slWaveMin) / DISP_HEIGHT * (DISP_HEIGHT + DISP_MARGIN*2) / DISP_HEIGHT;
			slWaveMax = 0;
			slWaveMin = 0x7fffff;
		}
	}
